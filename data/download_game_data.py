...
from src.constants import CONFIG_PATH, GAME_DATA_DIR, MC_DOWNLOADS_DIR, GAME_DATA_FILES
...
def check_mc_data(redownload: bool = False, delete=True) -> bool:
    ...
    # Check all version dirs in GAME_DATA_DIR
    matching_versions = False
    for version in os.listdir(resource_path(GAME_DATA_DIR)):
        if version.removeprefix(".") == latest_version:
    ...
    if matching_versions:
        ...
        if not os.path.exists(resource_path(GAME_DATA_DIR, latest_version)) or not all(
